...
class Members extends FlipBook {
  ...
  getData = async () => {
    try {
      //president
      Client.getEntries({ content_type: "president" }).then(response => {
      ...
      //vp student life
      Client.getEntries({ content_type: "vpStudentLife" }).then(response => {
        response.items.forEach(data => {
          this.vpStudentLifeData.push(data.fields);
        });
      });
      //vp academic
      Client.getEntries({ content_type: "vpAcademic" }).then(response => {
        response.items.forEach(data => {
          this.vpAcademicData.push(data.fields);
        });
      });
      //vp admin
      Client.getEntries({ content_type: "vpAdmin" }).then(response => {
        response.items.forEach(data => {
          this.vpAdminData.push(data.fields);
        });
      });
      //vp engagement
      Client.getEntries({ content_type: "vpEngagement" }).then(response => {
        response.items.forEach(data => {
      ...
      //vp finance
      Client.getEntries({ content_type: "vpFinance" }).then(response => {
        response.items.forEach(data => {
          this.vpFinanceData.push(data.fields);
        });
      });
      //amsRep
      Client.getEntries({ content_type: "amsRep" }).then(response => {
        response.items.forEach(data => {
  ...
  FlipPage = e => {
    ...
    if (e.target.id === "0") {
      ...
      this.setState({
        selectedMember: this.presidentData[0].name
      });
      ...
      this.setState({
        selectedMember: this.vpStudentLifeData[0].name
      });
      ...
      this.setState({
        selectedMember: this.vpAcademicData[0].name
      });
      ...
      this.setState({
        selectedMember: this.vpAdminData[0].name
      });
      ...
      this.setState({
        selectedMember: this.vpEngagementData[0].name
      });
      ...
      this.setState({
        selectedMember: this.amsRepData[0].name
      });
    }
  };
  ...
  render() {
    let memberPics = [];
    let memberPlatforms = this.state.candData;
    //resize/format pic based on data
    memberPlatforms.map((data, ind) => {
      //set scaling of img
      let scale =
    ...
    let page = (
      <>
        <h1>
          {this.state.title}
          {": "}
          {this.state.title.length + this.state.selectedMember.length > 40 ? (
            <br></br>
          ) : (
            ""
          )}
